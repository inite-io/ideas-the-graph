...
export class IniteNFT extends ethereum.SmartContract {
  static bind(address: Address): IniteNFT {
    return new IniteNFT("IniteNFT", address);
  }
  ...
  mintTo(recipient: Address): BigInt {
    let result = super.call("mintTo", "mintTo(address):(uint256)", [
  ...
  try_mintTo(recipient: Address): ethereum.CallResult<BigInt> {
    let result = super.tryCall("mintTo", "mintTo(address):(uint256)", [
      ethereum.Value.fromAddress(recipient)
    ]);
    if (result.reverted) {
      return new ethereum.CallResult();
